...
class TestPreviewJournalUpdate:
    """Tests for journal preview functionality."""
    ...
    def test_preview_shows_diff_without_modifying(
        ...
        original_content = sample_journal_files["today_file"].read_text()

        # Create a new entry object with modified content
        new_entry = server.JournalEntry(
            time=original_entry.time,
            headline="Modified headline for preview",
            tags=original_entry.tags,
            content="- New content for preview test",
            line_number=original_entry.line_number,
            file_date=original_entry.file_date,
        )

        preview_output = server.format_journal_preview(
            original_entry, new_entry
        )

        # Verify preview output contains expected markers
        assert "Preview:" in preview_output
        assert "Proposed changes:" in preview_output

        # Verify file was NOT modified
        after_content = sample_journal_files["today_file"].read_text()
        assert after_content == original_content

    def test_preview_shows_headline_change(
        self, sample_journal_files: JournalFilesInfo
    ) -> None:
        """Test that preview shows headline changes."""
        entries = server.parse_journal_entries(
            sample_journal_files["today_file"]
        )
        original_entry = entries[0]
        ...
        # Should show the headline change
        assert "−" in preview_output or "+" in preview_output

    def test_preview_shows_time_change(
        self, sample_journal_files: JournalFilesInfo
    ) -> None:
        """Test that preview shows time changes."""
        entries = server.parse_journal_entries(
            sample_journal_files["today_file"]
        )
        original_entry = entries[0]
        ...
        new_entry = server.JournalEntry(
            time="23:59",  # Changed time
            headline=original_entry.headline,
            tags=original_entry.tags,
            content=original_entry.content,
            line_number=original_entry.line_number,
            file_date=original_entry.file_date,
        ...
        # Should show the time change in diff
        assert "23:59" in preview_output
    ...
    def test_preview_no_changes(
        ...
        same_entry = server.JournalEntry(
            time=original_entry.time,
            headline=original_entry.headline,
            tags=original_entry.tags,
            content=original_entry.content,
