...
def main():
    if len(sys.argv) < 3:
        print("Usage: install_desktop.py <install|uninstall> <config_path> <mcp_name> [<uv_path> <root_dir> <server_script>]")
        sys.exit(1)
    ...
    if command == 'install':
        if len(sys.argv) != 7:
            print("Usage: install_desktop.py install <config_path> <mcp_name> <uv_path> <root_dir> <server_script>")
            sys.exit(1)
        uv_path = sys.argv[4]
        root_dir = sys.argv[5]
