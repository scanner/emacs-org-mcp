...
def format_task_preview(
    ...
    """Format a preview of task changes (diff only, no modification)."""
    lines = []
...
def format_journal_preview(
    ...
    """Format a preview of journal entry changes (diff only, no modification)."""
    lines = [
...
def format_move_result(
    ...
    """Format the result of moving a task."""
...
def parse_tasks_in_section(
    ...
    """Parse tasks that are direct children of the given section heading."""
    tasks: list[Task] = []
...
def find_task(
...
) -> tuple[Task, Heading, Heading, Org] | None:
    """
    Find a task by identifier (:CUSTOM_ID:, ticket ID, or headline substring).

    Returns (Task, heading, section_heading, org) tuple or None.
    """
    org = get_org()
    ...
    for sec_name in sections:
        ...
        for heading in section_heading.children:
            ...
            # Check matches
            matches = False
            if custom_id and custom_id == identifier:
                matches = True
            elif custom_id and custom_id == f"task-{identifier.lower()}":
                matches = True
            elif ticket_match := re.search(r"\b([A-Z]+-\d+)\b", headline_text):
                ticket = ticket_match.group(1)
                if ticket.upper() == identifier.upper():
                    matches = True
            elif identifier.lower() in headline_text.lower():
                matches = True
...
def parse_task_entry(task_entry: str) -> Heading:
    """
    Parse a task entry string (level 2 heading) into a Heading object.

    orgmunge requires org content to start with a level 1 heading,
    so we wrap the task entry in a dummy section and extract the child.
    """
...
def create_task(section_name: str, task_entry: str) -> tuple[str, str]:
    """Add a new task to the specified section. Returns (section_name, task_content)."""
    org = get_org()

    # Parse the new task entry
    new_heading = parse_task_entry(task_entry)
    ...
    if section_name == ACTIVE_SECTION:
        headline_title = (
            new_heading.headline.title
            if hasattr(new_heading.headline, "title")
            else str(new_heading.headline)
        )
        description = extract_task_description(headline_title)
        add_high_level_task(org, description)

    org.write(str(TASKS_FILE))
    ensure_trailing_newline(TASKS_FILE)

    # Return section and the task content for formatting
    return (section_name, heading_to_org_string(new_heading))
...
def update_task(
    ...
    Returns (old_task, new_content, was_moved, old_section, new_section).
    """
    result = find_task(identifier)
    if not result:
    ...
    if old_section_heading == target_section:
        ...
        try:
            ...
            raise ValueError("Could not find task heading in section")
        ...
        if len(current_children) > 1:
            new_heading_in_list = current_children[-1]
            current_children.pop()
            current_children.insert(idx, new_heading_in_list)
...
def move_task(
    ...
    """Move a task from one section to another. Returns (headline, from_section, to_section)."""
    result = find_task(identifier, from_section)
    if not result:
        raise ValueError(f"Task '{identifier}' not found in '{from_section}'")
...
def detect_journal_extension() -> str:
    ...
    Returns ".org" if any existing journal file has that extension, otherwise "".
    This ensures new files match the existing convention in the journal directory.
    """
...
def get_journal_path(target_date: date) -> Path:
    """Get journal file path for a date. Format: YYYYMMDD or YYYYMMDD.org"""
...
def parse_journal_entry(
    ...
    """Parse a single journal entry starting at start_idx."""
    match = re.match(
        r"^\*\*\s+(\d{2}:\d{2})\s+(.+?)(?:\s+:([^:]+(?::[^:]+)*):)?$",
        lines[start_idx],
    )
...
def create_journal_entry(
    ...
    """Create or append a journal entry. Returns (target_date, entry)."""
    ...
    if file_path.exists():
        existing = read_file(file_path).rstrip()
...
def update_journal_entry(
    ...
    """Update an existing journal entry by line number. Returns (old_entry, new_entry, date)."""
